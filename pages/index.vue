    ...
    try {
      ...
      if (fetchedRepos && fetchedRepos.ok) {
        ...
        for (let i = 0; i < projects.length; i += 1) {
          ...
          const repo = repos.find((el: any) => el.name === project.name)
          if (repo) {
  ...
  methods: {
    ...
    toggleSearch() {
      ...
      if (this.showSearch) {
        ...
        this.$nextTick(() => (this.$refs.search as any).focus())
      } else {
